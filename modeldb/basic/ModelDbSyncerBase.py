...
class Syncer(object):
    ...
    @classmethod
    def create_syncer(cls, proj_name, user_name, proj_desc=None, host=None, port=None):
        """
    ...
    def sync_datasets(self, datasets):
        ...
        if type(datasets) != dict:
            self.datasets["default"] = datasets
        else:
    ...
    def get_dataset_for_tag(self, data_tag):
        if data_tag not in self.datasets:
            ...
            print(data_tag, ' dataset not defined. default dataset will be used.')
            data_tag = "default"
